...
const ON_DEMAND_PER_ORG_PER_HOUR = 20; // moment + range
...
//    | { kind: "range", start, end, question?, tz?, uom? }
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  if (body.kind === "range") {
    const start = Number(body.start);
    const end = Math.min(Number(body.end), now);
    if (!Number.isFinite(start) || !Number.isFinite(end) || end - start < RANGE_MIN_MS)
      return NextResponse.json({ error: "Pick a range of at least 15 minutes" }, { status: 400 });
    if (end - start > RANGE_MAX_MS + 3600_000)
      return NextResponse.json({ error: "Nova can review up to 31 days at a time — pick a shorter range" }, { status: 400 });
    const limited = await onDemandLimit(ctx.orgId, now);
    ...
    return runAndStore(data.id, () => analyzeRange(ctx, cam, { start, end, question, tz, uom, ref: `camera:${cam.id};insight:${data.id}` }));
  }
  ...
  return NextResponse.json({ error: 'kind must be "daily", "moment" or "range"' }, { status: 400 });
}

async function onDemandLimit(orgId: string, now: number) {
  const { count } = await db()
    .from("camera_insights")
    .select("id", { count: "exact", head: true })
    .eq("org_id", orgId)
    .in("kind", ["moment", "range"])
    .gte("created_at", new Date(now - 3600_000).toISOString());
  if ((count ?? 0) >= ON_DEMAND_PER_ORG_PER_HOUR)
